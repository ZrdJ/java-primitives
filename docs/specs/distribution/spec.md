...
and tested against the range of JDKs the library targets.
...
`junit`, `assertj-core` and `org.jusecase:builders` as test-scoped
dependencies and `src/test` holds 14 JUnit test files, so `verify` both
compiles/packages the artifact and runs that suite. A further push to the
same branch must cancel the still-running build for the previous push on
that branch.
...
- **THEN** `mvn -B -ntp verify` runs once under JDK 11, once under JDK 17
  and once under JDK 21, compiling and running the test suite each time
...
`main`. The Maven coordinates `com.github.zrdj:java-primitives` resolve on
JitPack against this repository, `ZrdJ/java-primitives`.
...
- **THEN** a build for tag `X` is requested from JitPack at
  `jitpack.io/com/github/ZrdJ/java-primitives/X/build.log`
